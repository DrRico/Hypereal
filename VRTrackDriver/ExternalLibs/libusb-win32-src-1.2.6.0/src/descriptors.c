...
static int usb_parse_endpoint(struct usb_endpoint_descriptor *endpoint, unsigned char *buffer, int size)
{
    ...
    {
        if (usb_debug >= 1)
			debug_print("ran out of descriptors parsing\n");
        return -1;
    }
    ...
    {
        if (usb_debug >= 2)
			debug_print("unexpected descriptor 0x%X, expecting endpoint descriptor, type 0x%X\n",
    ...
    while (size >= DESC_HEADER_LENGTH)
    {
        ...
        {
            if (usb_debug >= 1)
				debug_print("invalid descriptor length of %d\n", header.bLength);
            return -1;
        }
        ...
        if (usb_debug >= 1)
			debug_print("skipping descriptor 0x%X\n", header.bDescriptorType);
        numskipped++;
    ...
    if (numskipped && usb_debug >= 2)
		debug_print("skipped %d class/vendor specific endpoint descriptors\n", numskipped);
    ...
    {
        if (usb_debug >= 1)
			debug_print("couldn't allocate memory for endpoint extra descriptors\n");
        endpoint->extralen = 0;
        return parsed;
    }
                               ...
                               unsigned char *buffer, int size)
{
    ...
    while (size >= INTERFACE_DESC_LENGTH)
    {
        ...
        {
            if (usb_debug >= 1)
				debug_print("couldn't malloc interface->altsetting\n");
            return -1;
        }
        ...
        if (numskipped && usb_debug >= 2)
			debug_print("skipped %d class/vendor specific interface descriptors\n", numskipped);
        ...
        else
        {
            ...
            {
                if (usb_debug >= 1)
					debug_print("couldn't allocate memory for interface extra descriptors\n");
                ifp->extralen = 0;
                return -1;
            }
        ...
        {
            if (usb_debug >= 1)
				debug_print("too many endpoints\n");
            return -1;
        }
        ...
        if (ifp->bNumEndpoints > 0)
        {
            ...
            {
                if (usb_debug >= 1)
					debug_print("couldn't allocate memory for ifp->endpoint\n");
                return -1;
            }
                            ...
                            unsigned char *buffer)
{
    ...
    {
        if (usb_debug >= 1)
			debug_print("too many interfaces\n");
        return -1;
    }
    ...
    for (i = 0; i < config->bNumInterfaces; i++)
    {
        ...
        if (len)
        {
            ...
            if (!config->extralen)
            {
                ...
                {
                    if (usb_debug >= 1)
						debug_print("couldn't allocate memory for config extra descriptors\n");
                    config->extralen = 0;
                    return -1;
                }
...
void usb_fetch_and_parse_descriptors(usb_dev_handle *udev)
{
    ...
    {
        if (usb_debug >= 1)
			debug_print("Not enough configurations (%d < %d)\n", dev->descriptor.bNumConfigurations, 1);
        return;
    }
    ...
    {
        if (usb_debug >= 1)
			debug_print("Unable to allocate memory for config descriptor\n");
        return;
    }
    ...
    for (i = 0; i < dev->descriptor.bNumConfigurations; i++)
    {
        ...
        if (res < USB_DT_CONFIG_SIZE)
        {
            ...
            {
                if (res < 0)
					debug_print("Unable to get descriptor (%d)\n", res);
                else
					debug_print("Config descriptor too short (expected %d, got %d)\n", USB_DT_CONFIG_SIZE, res);
            }
        ...
        {
            if (usb_debug >= 1)
				debug_print("Unable to allocate memory for descriptors\n");
            goto err;
        }
        ...
        {
            if (res > 0)
				debug_print("Descriptor data still left\n");
            else if (res < 0)
				debug_print("Unable to parse descriptors\n");
        }
