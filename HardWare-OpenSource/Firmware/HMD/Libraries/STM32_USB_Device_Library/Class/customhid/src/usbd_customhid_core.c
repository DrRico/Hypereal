...
__ALIGN_BEGIN static uint8_t CustomHID_ReportDesc[USBD_CUSTOM_HID_REPORT_DESC_SIZE] __ALIGN_END =
{
  ...
  /* ADC IN */
  0x85, ADC_REPORT_ID,   /*     REPORT_ID     0x07            */         
...
extern uint8_t bulk_rx_buf[64];
extern uint8_t bulk_rx_count;
extern uint8_t bulk_rx_flag;
                                 ...
                                 uint8_t cfgidx)
{
  ...
  DCD_EP_Close (pdev , HID_OUT_EP);
   DCD_EP_Close (pdev , HID_IN_BULK_EP);
  DCD_EP_Close (pdev , HID_OUT_BULK_EP); 
...
uint8_t USBD_CUSTOM_bulk_send     (USB_OTG_CORE_HANDLE  *pdev, 
                                 uint8_t *report,
                                 uint16_t len)
{
  if (pdev->dev.device_status == USB_OTG_CONFIGURED )
  {
    DCD_EP_Tx (pdev, HID_IN_BULK_EP, report, len);
  }
  return USBD_OK;
}
                              ...
                              uint8_t epnum)
{
  ...
  DCD_EP_Flush(pdev, HID_IN_BULK_EP);
...
/**
  ...
  * @retval status
  */
uint8_t usb_hid_data_out_flag = 0;
uint8_t usb_hid_data_out_data[64];
                                  ...
                                  uint8_t epnum)
{
  ...
  if (epnum == 1)
	{
			memcpy(usb_hid_data_out_data,Report_buf,2);
			usb_hid_data_out_flag = 1;
	}
	else	//epnum == 2
	{
		bulk_rx_count = USB_Rx_Cnt;
		bulk_rx_flag = 1;
		
		memcpy(bulk_rx_buf,bulk_rx_buf_t,bulk_rx_count);
	}
